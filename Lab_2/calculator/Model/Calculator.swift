...
public protocol Calculator: class { // можно реализовывать только в ссылочном типе
    ...
    /// Максимальное количество текущих знаков после запятой в правом значении
    var maxFractDigits: UInt { get }
    
    var maxInpLen: UInt { get }
    
    // Унарная операция
    func unarOperation(_ op: Operation)
